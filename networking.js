...
class Peer {
    constructor(id, localId, signaler, onPeerMsg, onNewPeerReady) {
        this.ready = false; // ready to send p2p messages
...
class Networking {
    ...
    constructor() {
        this.peers = []; // a host peer will have every peer in the game but a client will only connect to the host
        this.gamesList = [];
        this.onPeerMsg = (x) => { };
        this.onNewPeer = (x) => { };
        this.onServerOpen = () => { };
        this.visable = false;
        this.hosting = false; // wether we are the host of the game
        this.connected = false; // in a game rn
    ...
    remoteFromId(id, dontCreate = false) {
        ...
        if (temp.length >= 1) {
            ...
            const signaler = (response) => { this.wsSend("rtc-signal", response); };
    ...
    joinGame(gameId) {
        ...
        if (!this.id) {
            console.log("tried to join a game before getting id");
            return;
        }
        this.hosting = false;
        this.visable = false;
