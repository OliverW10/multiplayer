...
export class Game {
    constructor() {
        ...
        this.players = [];
        this.map = [];
        this.viewPos = new Rect(0, 0, 999, 0.2);
        this.VIEW_MARGIN = 0.1; // draw things up to 0.2 outside view to prevent popin 
        this.outerViewPos = new Rect(0, 0, 999, 0.3);
        this.clientTickRate = 30;
        this.lastTickTime = 0; // time since last tick
        this.inputX = 0; // rotation input + is clockwise
        this.inputY = 0; // forwards input + is forwards
        this.lookAngle = 0;
    ...
    render(ctx) {
        ...
        this.outerViewPos = new Rect(this.viewPos.x - this.VIEW_MARGIN / 2, this.viewPos.y - this.VIEW_MARGIN / 2, this.viewPos.w + this.VIEW_MARGIN, this.viewPos.h + this.VIEW_MARGIN);
        // same here
    ...
    drawMinimap() {
        ...
        ctx.beginPath();
        ctx.fillStyle = "rgba(200, 200, 200, 0.8)";
        ctx.rect(minimapRect.x + this.viewPos.x * minimapRect.w, minimapRect.y + this.viewPos.y * minimapRect.h, minimapRect.w * this.viewPos.w, minimapRect.h * this.viewPos.h);
        ctx.fill();
        for (let p of this.players) {
            // render them on minimap
            ctx.beginPath();
            ctx.fillStyle = "rgba(255, 0, 0, 1)";
            ctx.rect(minimapRect.x + p.pos.x * minimapRect.w, minimapRect.y + p.pos.y * minimapRect.h, 10, 10);
            ctx.fill();
        }
        // border
        ctx.beginPath();
    ...
    drawMap() {
        ...
        for (let line of this.map) {
            if ((line.p1.x > this.outerViewPos.x && line.p1.x < this.outerViewPos.x + this.outerViewPos.w &&
                ...
                (line.p2.x > this.outerViewPos.x && line.p2.x < this.outerViewPos.x + this.outerViewPos.w &&
                    line.p2.y > this.outerViewPos.y && line.p2.y < this.outerViewPos.y + this.outerViewPos.h)) { // only run for lines within view + viewMargin
    ...
    playerDist(pos, fast = false) {
        // fast dosent do the sqrt because if all you care about is relative distance you dont need it
        if (this.us) {
            const ourPos = this.us.pos;
            if (fast) {
                return Math.pow((ourPos.x - pos.x), 2) + Math.pow((ourPos.y - pos.y), 2);
            }
            else {
                return Math.sqrt(Math.pow((ourPos.x - pos.x), 2) + Math.pow((ourPos.y - pos.y), 2));
            }
        }
        return 999;
    }
    ...
    getInput(forSend = true) {
        ...
        if (this.grabbing) {
            obj.data["swinging"] = true;
        }
        if (this.shooting) {
            obj.data["shooting"] = true;
        }
        if (this.detonating) {
            obj.data["detonating"] = true;
        }
        if (forSend) {
            this.inputX = 0;
            this.inputY = 0;
...
function createGameList(list, ourId) {
    ...
    for (let id of list) {
        ...
        (_a = document.getElementById("gameList")) === null || _a === void 0 ? void 0 : _a.appendChild(trNode);
    }
}
