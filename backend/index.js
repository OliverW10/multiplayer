...
function accept(req, res) {
  ...
  if (!(req.headers.connection.match(/\bupgrade\b/i) || req.headers.connection.match(/\bkeep-alive\b/i))) {
    res.end();
    return;
  }
...
let clients = [];
const randomId = (size=6) =>Math.floor(Math.random()*(10**size - 10**(size-1)) + 10**(size-1));
...
function onConnect(ws) {
  ...
  console.log(`client connected, all clients: ${clients}`);
  ...
  ws.on('message', function (message) {
    ...
    switch(messageObj.type){
      case 'get-id':
        ...
        ws.send(JSON.stringify({ type:"give-id", data:getClientFromWs(ws).id }))
        break;
      case 'list-games':
        ws.send(JSON.stringify( {type:"games-list", data:clients.map(c=>c.id)} ))
        break;
